...
class SubjectPricing:
    ...
    def __init__(self, download=False):
        ...
        self.subject_pricing_file = os.path.join(
            parent_path, self.subject_pricing_file_name
        )
        self.get_sheet()
    ...
    def get_sheet(self):
        if self.download:
            self.download_file_from_google_drive(
                "https://docs.google.com/spreadsheets/d/1BBI6HUCpHkHk_AgxBEGACpL90dV_D4mySG3-c0ewGVY",
                # "https://docs.google.com/spreadsheets/d/1gQePBgA7f6ixjov1ivwLsf5jVUvqn5jdFD4zZOhY65Y",
    ...
    def get_all_states_with_vicinities(self):
        ...
        for x in range(start_row, self.sheet.max_row):
            if self.sheet.cell(row=x, column=state_col).value != temp_state:
                states.append({"state": temp_state, "vicinities": temp_vicinities})
                temp_state = self.sheet.cell(row=x, column=state_col).value
                temp_vicinities = []
            factor = self.sheet.cell(row=x, column=vicinity_fx_col).value or 1
            try:
                temp_vicinities.append(
                    {
                        "name": self.sheet.cell(row=x, column=vicinity).value,
                        "factor": factor * 100.0,
                    }
                )
            except:
    ...
    def get_vicinity_factor(self, vicinity):
        ...
        vicinities = [
            ...
            if vicinity and v["name"].lower().strip() == vicinity.lower().strip()
        ]
            ...
            purposes.append(
                {
                    ...
                    "factor": round(
                        self.sheet.cell(row=x, column=purpose_factor + 1).value * 100, 1
                    ),
                }
            )
        return purposes
    ...
    def get_all_subjects_and_their_prices(self):
        ...
        for x in range(start_row, self.sheet.max_row):
            if (
                not self.sheet.cell(row=x, column=1).value
                and not self.sheet.cell(row=x + 1, column=1).value
            ):
                break
    ...
    def update_curriculums(self, purposes, curriculums):
        curriculums = set(curriculums or [])
        purpose_curriculum_relation = self.get_purpose_curriculum_relation()
        purpose_and_curriculums = [
            list(x.values()) for x in purpose_curriculum_relation
        ]
            ...
            curriculums.append(
                {
                    ...
                    "factor": self.sheet.cell(row=x, column=curriculum_col + 1).value
                    * 100,
                }
            )
        return curriculums
    ...
    def get_all_hours_and_factors(self):
        ...
        for x in range(start_row, self.sheet.max_row):
            ...
            hours.append(
                {
                    "hours": float(
                        self.sheet.cell(row=x, column=hour_col).value.split(" ")[0]
                    ),
    ...
    def get_all_plans(self):
        start_row = 9
        hours = []
        hour_col = self.find_column_num()["curriculum"]
        for x in range(start_row, self.sheet.max_row):
            if not self.sheet.cell(row=x, column=hour_col).value:
                break
            hours.append(
                {
                    "name": self.sheet.cell(row=x, column=hour_col).value,
                    "factor": self.sheet.cell(row=x, column=hour_col + 1).value,
                }
            )
        return hours

    def get_all_exam_factors(self):
        start_row = 19
        hours = {}
        hour_col = self.find_column_num()["purpose_factor"]
        for x in range(start_row, self.sheet.max_row):
            if not self.sheet.cell(row=x, column=hour_col).value:
                break
            key = (self.sheet.cell(row=x, column=hour_col).value).split()
            key = "".join([k[0] for k in key])
            hours[key] = self.sheet.cell(row=x, column=hour_col + 1).value
            ...
            relation.append(
                {
                    ...
                    "curriculum": self.sheet.cell(
                        row=x, column=purpose_factor + 1
                    ).value,
                }
            )
        return relation
    ...
    def get_plan_factor(self, plan="plan 1"):
        plans = self.get_all_plans()
        factors = [x["factor"] for x in plans if x["name"].lower() == plan.lower()]
        return factors[0] if len(factors) > 0 else 1
    ...
    def get_marketing_channels(self):
        ...
        for x in range(start_row, self.sheet.max_row):
            if not self.sheet.cell(row=x, column=marketing_ch_col).value:
                break
            marketing_channels.append(
                self.sheet.cell(row=x, column=marketing_ch_col).value
            )
    ...
    def calculate_hourly_price(
        ...
        hourly_price = (
            base_rate
            * (purpose_factor / (100 ** (len(purposes))))
            * (curriculum_factor / 100)
            * (vicinity_factor / 100)
            * (state_factor / 100)
        )
    ...
    def get_hourly_price_and_transport(
        self,
        students=[],
        state="Lagos",
        vicinity=None,
        ...
        hourly_price, transport_fare = self.calculate_hourly_price(
            students, state, vicinity, curriculums, no_of_hours, subject
        )
        return {"hourly_price": hourly_price, "transport_fare": transport_fare}
