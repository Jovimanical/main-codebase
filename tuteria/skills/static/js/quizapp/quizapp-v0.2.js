...
//			qnaObj.question = "<b>"+qnaObj.id+": " + qnaArr[0] + "</b>";
...
function Quiz() {
    ...
    this.percentage = function(){
        return Math.round( this.score * 100 / this.totalQuestions);
    };
    this.passed = function(cutoff){
        return this.percentage() >= cutoff;
    }
}
...
quizApp.directive("iquestion", ['QuizService', function (QuizService) {
    return {
        ...
        controller: function ($scope) {
            ...
            $scope.evalScore = function (id, value) {
                ...
                if ($scope.started === false) {
                    $scope.started = true;
                    $scope.$emit('quizStarted', $scope.started);
                }
            };
        ...
        link: function (scope, element, attrs) {
            ...
            // Processing the question and answer text
            //
            qnaObj.question = "<b>" + qnaObj.id + ": " + text.content + "</b>";
            qnaObj.options = [];
            qnaObj.answers = [];
            for (i = 0; i < text.answer_set.length; i++) {
                var option = {
                    "text": text.answer_set[i].content,
                    "correct": text.answer_set[i].correct,
                    "style": ""
                };
                qnaObj.options.push(option);
            }
