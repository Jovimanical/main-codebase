...
/***/ function(module, exports, __webpack_require__) {
	...
	var Urls = __webpack_require__(5);
	var Utils = __webpack_require__(3);
	var parsleyConfig = Utils.parsleyConfig;
	var lodash_max = __webpack_require__(462);
	
	function increment(index, value) {
	  console.log(value);
	  var el = $(value);
	  var start_id = '#id_schedule_set-' + index + '-start_time',
	      end_id = '#id_schedule_set-' + index + '-end_time';
	  el.attr('id', 'schedule_set-' + index + '-row');
	  el.find('.day_of_week').attr('id', 'id_schedule_set-' + index + '-day').attr('data-parsley-error-message', "You must select a day from the checkboxes above").attr('name', 'schedule_set-' + index + '-day').parsley(parsleyConfig);
	  el.find('input.start_time').attr('id', 'id_schedule_set-' + index + '-start_time').attr('name', 'schedule_set-' + index + '-start_time').parsley(parsleyConfig);
	  el.find('input.end_time').attr('id', 'id_schedule_set-' + index + '-end_time').attr('data-parsley-schedule_time', start_id).attr('name', 'schedule_set-' + index + '-end_time').parsley(parsleyConfig);
	  el.find('input.delete_form').attr('id', 'id_schedule_set-' + index + '-DELETE').attr('name', 'schedule_set-' + index + '-DELETE');
	  el.find('input.id_form').attr('id', 'id_schedule_set-' + index + '-id').attr('name', 'schedule_set-' + index + '-id');
	
	  console.log(el);
	}
	
	function incrementForms() {
	  var schedules = $('#schedule-form .form-base'),
	      schedule_formset = $('#schedule-form');
	  $.each(schedules, increment);
	  schedule_formset.find('.add-row').hide();
	  $('#id_schedule_set-TOTAL_FORMS').val(schedules.length);
	}
	function hideAddMore(fset, parentNode) {
	  if (fset.length >= 2) {
	    parentNode.hide();
	  }
	  fset.find('.delete-row').hide();
	}
	
	function populateDayInput(node) {
	  if (node.val() === "") {
	    node.val($(this).parent('label').text().trim());
	  }
	}
	
	var education_fields = ['school', 'course', 'degree', 'id'];
	var we_fields = ['name', 'role', 'id'];
	function _added() {
	  $('#credential-form').parsley(parsleyConfig).reset();
	}
	function Credentials(j_form) {
	  var form = j_form,
	      we_formset = $('#we-form'),
	      education_formset = $('#education-form'),
	      guarantor_formset = $('#guarantor-form');
	  var isOperaMini = !!window['operamini'];
	  if (isOperaMini) {
	    education_formset.find('.checkbox').hide();
	    we_formset.find('.checkbox').hide();
	    guarantor_formset.find('.checkbox').hide();
	  } else {
	    Utils.parsleyfyForm(form, parsleyConfig);
	  }
	
	  education_formset.find('.form-base').formset({ addText: 'another school?',
	    prefix: 'education_set',
	    added: function added() {
	      var new_node = $($('#education-form .form-base')[1]);
	      new_node.attr('id', 'education_set-1-row').find('#id_education_set-1-id').remove();
	      new_node.find('input').removeData();
	      new_node.find('select').removeData();
	      _added();
	    }
	  });
	
	  we_formset.find('.form-base').formset({ prefix: 'workexperience_set',
	    added: function added() {
	      var new_node = $($('#we-form .form-base')[1]);
	      new_node.attr('id', 'workexperience_set-1-row').find('#id_workexperience_set-1-id').remove();
	      new_node.find('input').removeData();
	      new_node.find('select').removeData();
	      _added();
	    } });
	  guarantor_formset.find('.form-base').formset({ addText: 'another guarantor?',
	    prefix: 'guarantor_set',
	    added: function added() {
	      var new_node = $($('#guarantor-form .form-base')[1]);
	      new_node.attr('id', 'guarantor_set-1-row').find('#id_guarantor_set-1-id').remove();
	      new_node.find('input').removeData();
	      new_node.find('select').removeData();
	      _added();
	    }
	  });
	  var max_no_of_forms = 2;
	
	  var education_add_row = education_formset.find('.add-row'),
	      we_add_row = we_formset.find('.add-row'),
	      guarantor_add_row = guarantor_formset.find('.add-row');
	
	  hideAddMore($('#education-form .form-base'), education_add_row);
	  hideAddMore($('#we-form .form-base'), we_add_row);
	  hideAddMore($('#guarantor-form .form-base'), guarantor_add_row);
	
	  education_formset.find('.add-row').on('click', function () {
	    hideAddMore($('#education-form .form-base'), $(this));
	  });
	  we_formset.find('.add-row').on('click', function () {
	    hideAddMore($('#we-form .form-base'), $(this));
	  });
	  guarantor_formset.find('.add-row').on('click', function () {
	    hideAddMore($('#guarantor-form .form-base'), $(this));
	  });
	  function inputCurriculum(addr_state) {
	    if (addr_state) {
	      $('#curriculum_pref').removeClass('hidden');
	      $('#id_curriculum_explaination').attr('required', true).attr('disabled', false);
	    } else {
	      $('#curriculum_pref').addClass('hidden');
	      $('#id_curriculum_explaination').attr('disabled', true).removeAttr('required');
	    }
	  }
	  function ImplementClickEvent(my_id, second_id, third_id) {
	    $('#id_curriculum_used_' + my_id).change(function () {
	      var $this = $(this);
	      console.log($this);
	      if ($this.is(':checked')) {
	        inputCurriculum(true);
	      } else {
	        if ($('#id_curriculum_used_' + second_id).is(':checked') || $('#id_curriculum_used_' + third_id).is(':checked')) {
	          inputCurriculum(true);
	        } else {
	          inputCurriculum(false);
	        }
	      }
	    });
	  }
	  $('#parsley-id-multiple-classes').addClass('hidden');
	  $('#parsley-id-multiple-curriculum_used').addClass('hidden');
	  ImplementClickEvent(1, 1, 3);
	  ImplementClickEvent(2, 1, 3);
	  ImplementClickEvent(3, 1, 2);
	}
	
	function PreferenceForm(form, loc_distances, callback) {
	  function MonthlyBookingExplanation(val) {
	    if (val) {
	      $('.monthly_booking_checkbox').text('All your subjects would be set to monthly booking. You would be required to change individual subjects manually ');
	    } else {
	      $('.monthly_booking_checkbox').text('All your subjects have been reverted to hourly bookings');
	    }
	  }
	  ...
	  // $('#id_allow_monthly').on('click', function () {
	  //   var display_group = form.find('.disable-group');
	  //   console.debug(display_group);
	  //   if (!$(this).is(':checked')) {
	  //     $('#monthly_section').addClass("hidden")
	  //     // form.parsley(parsleyConfig)
	  //     // display_group.attr('readonly', true);
	  //     // display_group.attr('disabled', true);
	
	  //     if (callback) {
	  //       MonthlyBookingExplanation(false);
	  //     }
	  //   }
	  //   else {
	  //     $('#monthly_section').removeClass("hidden")
	  //     // form.parsley(parsleyConfig);
	  //     // display_group.attr('readonly', false);
	  //     // display_group.attr('disabled', false);
	  //     if (callback) {
	
	  //       MonthlyBookingExplanation(true);
	  //     }
	  //   }
	  // });
	
	  var tutoring_dist = $('#id_tutoring_distance');
	  var dist_text = $('#your-location');
	  console.log(loc_distances);
	  if (!$.isEmptyObject(loc_distances)) {
	    TutoringDistanceText(loc_distances[tutoring_dist.val()]);
	  }
	  tutoring_dist.on('change', function () {
	    var tag = $(this).find("option:selected");
	    if (!$.isEmptyObject(loc_distances)) {
	      var section = loc_distances[tag.val()];
	      var selected = lodash_max(section, function (v) {
	        return parseFloat(v.distance);
	      });
	      console.log(selected);
	      TutoringDistanceText(section, tag);
	    }
	  });
	  function TutoringDistanceText(section, tag) {
	    var text = "from your address to ";
	    if (section) {
	      $.each(section, function (index, v) {
	        text = text + v.name;
	      ...
	      if (section.length > 0) {
	        dist_text.html("<em>For Example: </em> " + text);
	      } else {
	        dist_text.text($('#id_tutoring_distance option:selected').text());
	      }
	    } else {
	      dist_text.text($('#id_tutoring_distance option:selected').text());
	    }
	  ...
	  Utils.parsleyfyForm(form, parsleyConfig);
	  inputTutoringAddress(false);
	  $('#id_tutoring_address').on('change', function () {
	    var tag = $(this).find("option:selected");
	    if (tag.val() === 'user') {
	      TutoringDistanceText(loc_distances[tutoring_dist.val()]);
	      tutoring_dist.attr('disabled', false);
	      inputTutoringAddress(false);
	    } else {
	      tutoring_dist.attr('disabled', true);
	    }
	    if (tag.val() === 'tutor') {
	      dist_text.text("Clients would be required to come over to your location.");
	      inputTutoringAddress(true);
	    }
	    if (tag.val() === 'neutral') {
	      dist_text.text("You and clients would agree on where best to meet.");
	      inputTutoringAddress(false);
	    }
	  });
	  function inputTutoringAddress(addr_state) {
	    if (addr_state) {
	      $('#tutoring_address_section').removeClass('hide');
	      $('#id_address_reason').attr('required', true).attr('disabled', false);
	    } else {
	      $('#tutoring_address_section').addClass('hide');
	      $('#id_address_reason').attr('disabled', true).removeAttr('required');
	    }
	  }
	
	  $('#id_booking_prep').on('change', function () {
	    var tag = $(this).find("option:selected");
	  });
	}
	
	exports.increment = increment;
	exports.incrementForms = incrementForms;
	exports.PreferenceForm = PreferenceForm;
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 */
	var baseCallback = __webpack_require__(463),
	    baseEach = __webpack_require__(472),
	    isIterateeCall = __webpack_require__(473),
	    toIterable = __webpack_require__(474),
	    gt = __webpack_require__(476),
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * Available under MIT license <https://lodash.com/license>
	 */
	var baseIsEqual = __webpack_require__(464),
	    bindCallback = __webpack_require__(470),
	    isArray = __webpack_require__(465),
	    pairs = __webpack_require__(471);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isArray = __webpack_require__(465),
	    isTypedArray = __webpack_require__(466),
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * Available under MIT license <https://lodash.com/license>
	 */
	var getNative = __webpack_require__(468),
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Registration = __webpack_require__(461);
	__webpack_require__(6);
	__webpack_require__(11);
	__webpack_require__(12);
	__webpack_require__(160);
	
	window.initializeCallback = function GMapsCallbackfunction() {
	    function getAddress(autocomplete) {
	        var place = autocomplete.getPlace();
	        var state = place.address_components[place.address_components.length - 2].long_name;
	        console.debug(state);
	        var optionTag = Utils.filterOptionByText($('#id_location_set-0-state'), state);
	        optionTag.attr("selected", true);
	        console.log(optionTag);
	    }
	
	    Utils.GoogleAutoComplete($('#id_location_set-0-address'), getAddress);
	};
	$(function () {
	    var isOperaMini = !!window['operamini'];
	    if (!isOperaMini) {
	    ...
	    if (window.ProfileStep === 1) {
	        $('#id_location_set-0-DELETE').closest('.form-group').addClass("hidden");
	        var totalChars = 70; //Total characters allowed in textarea
	        var countTextBox = $('#id_description'); // Textarea input box
	        var descriptionCounter = $('small.char-count em');
	        countTextBox.keyup(function () {
	            descriptionCounter.text($(this).val().length);
	            // Utils.keyupCallback(this,500,descriptionCounter);
	        });
	        var attr = {
	            language_url: '',
	            verified_status: window.phoneNumberIsVerified,
	            verify_phone_url: Urls["users:verify_primary_number"]()
	        };
	        Initialize(attr);
	    }
	    if (window.ProfileStep === 2) {
	        var image_container = $('#image_container');
	        //console.log(isOperaMini);
	        if (isOperaMini === false) {
	            $('#edit_media_form input[type="file"]')
	            // .fileupload({
	            .cloudinary_fileupload({
	                dropZone: '#edit_media_form',
	                start: function start() {
	                    //$('#image-upload-btn').button('loading');
	                    $('.status_value').removeClass('hide').text('Starting direct upload...');
	                },
	                progress: function progress() {
	                    $('.status_value').text('Uploading...');
	                },
	                fail: function fail() {
	                    $('.sttus_value').text('Failed to Upload.');
	                }
	            })
	            ...
	            .off("cloudinarydone").on('cloudinarydone', function (e, data) {
	                ...
	                console.debug($(this.form).serialize());
	                $.post(this.form.action, $(this.form).serialize()).always(function (result, status, jqxhr) {
	                    ...
	                    if (status === "success") {
	                        //$('#image-upload-btn').button('reset');
	                        //if(window.TutorIntent){
	                        //    window.location.href=Urls["users:edit_verification"]();
	                        //}else{
	                        window.location.reload();
	                        //}
	                    }
	                });
	                image_container.html('').append($.cloudinary.image(data.result.public_id, {
	            ...
	            var beforeSubmit = function beforeSubmit() {
	                ...
	                successCallback = function successCallback(e, r) {
	                console.log(e);
	                console.log(r);
	                $('#upload_btn').button('reset');
	                window.location.reload();
	            },
	                errorCallback = function errorCallback(e, r) {
	                console.log(e);
	                console.log(r);
	                $('#upload_btn').button('reset');
	            };
	            Utils.formSubmission(video_form, beforeSubmit, successCallback, errorCallback);
	        }
	        var video_form = $('#video-upload-form');
	        video_form.parsley();
	    }
	    if (window.ProfileStep === 3) {
	        if (!isOperaMini) {
	            $('#identification_form input[type="file"]').cloudinary_fileupload({
	                dropZone: '#identification_form',
	                start: function start() {
	        ...
	        var checkboxes = $("#request_bg_form input[type='checkbox']"),
	            submitButt = $("#request_bg_form button");
	...
	function Initialize(attr) {
	    var year_tag = $('#id_dob_year'),
	        month_tag = $('#id_dob_month'),
	        day_tag = $('#id_dob_day'),
	        cover_tag = "<div class='form-group'></div>",
	        multiple_lang = $('#id_languages'),
	        select_container = $('#select-container'),
	        unverified_primary_phone = $('.phone-verification'),
	        verified_primary_phone = $('.phone-numbers-container'),
	        states_tag = $('#id_location_set-0-state'),
	        verify_btn = $('#verify_btn'),
	        form = $('#edit_profile_form');
	
	    function changeFirstOptionElements() {
	        month_tag.find('option[value=0]').text("Select Month").val("");
	        year_tag.find('option[value=0]').text("Select Year").val("");
	        day_tag.find('option[value=0]').text("Select Day").val("");
	        states_tag.find('option').first().text("Select State");
	    }
	
	    function addFormControlToDOB() {
	        year_tag.addClass('form-control').attr('required', true).attr('data-parsley-group', 'default').attr('data-parsley-error-message', 'Select a year').wrap(cover_tag);
	        month_tag.addClass('form-control').attr('required', true).attr('data-parsley-group', 'default').wrap(cover_tag).attr('data-parsley-error-message', 'Select a month').wrap(cover_tag);
	        day_tag.addClass('form-control').attr('required', true).attr('data-parsley-group', 'default').attr('data-parsley-error-message', 'Select a day').wrap(cover_tag);
	    }
	
	    function TogglePhoneVerification(verified_status) {
	        console.log(verified_status.toLowerCase());
	        if (verified_status.toLowerCase() === "false") {
	            unverified_primary_phone.removeClass("hidden");
	            verified_primary_phone.addClass("hidden");
	        } else {
	            unverified_primary_phone.addClass("hidden");
	            verified_primary_phone.removeClass("hidden");
	            $('#id_primary_phone_no1').val($('#id_number').val());
	        }
	    }
	
	    changeFirstOptionElements();
	    addFormControlToDOB();
	    form.removeClass('invisible');
	    // $('#id_number').intlTelInput({
	    //     utilsScript: "/static/js/intl-tel-input/lib/utils.js" // just for formatting/placeholders etc
	    // });
	    $('#id_number').attr('data-parsley-remote-message', "Phone number already exists");
	
	    var isOperaMini = !!window['operamini'];
	    $.listen('parsley:field:error', function (val) {
	        console.log(val);
	        // This global callback will be called for any field that fails validation.
	        console.log('Validation failed for: ', this.$element);
	    });
	    Registration.Credentials(form, true);
	    Registration.PreferenceForm(form, {});
	    ...
	    $('#id_tutor_intent').change(function () {
	        if ($(this).is(":checked")) {
	            $('#tutor-specific-form').removeClass("hidden").removeClass("hidden");
	            $('#id_tutor_description').attr('required', true);
	        } else {
	            $('#tutor-specific-form').removeClass("hidden").addClass("hidden");
	            $('#id_tutor_description').removeAttr('required');
	        }
	    });
	    if (!isOperaMini) {
	        Utils.parsleyfyForm(form);
	    };
	    TogglePhoneVerification(attr.verified_status);
	    var errorCallback = function errorCallback(xhr, err) {
	        var error = xhr.responseJSON.errors;
	    };
	    //Handles form submission
	    if (!isOperaMini) {
	        // Utils.formSubmission(form, function (parsedForm) {
	    ...
	    //Confirmation of Primary Phone number
	    verify_btn.on('click', function (e) {
	        e.preventDefault();
	        form.parsley().asyncValidate("phone_verify").done(function () {
	            console.log("validation 2 success");
	            form.parsley().validate("phone_verify");
	            if (form.parsley().isValid('phone_verify', true)) {
	                var successCallback = function successCallback(data) {
	                    console.log(data);
	                    $btn.button('reset');
	                    unverified_primary_phone.addClass('hidden');
	                    verified_primary_phone.removeClass('hidden');
	                    $('#primary_verified_no').text($('#id_number').val());
	                };
	
	                var _errorCallback = function _errorCallback(error, xhr) {
	                    console.log(xhr);
	                    console.log(error);
	                };
	    ...
	    //Toggling Primary Phone number for editing
	    ...
	    $('select[name*=state]').change(function (e) {
	        console.log("hello");
	        fetch(window.Urls['users:get_region']() + '?state=' + e.target.value, {
	            method: 'get'
	        }).then(function (response) {
	            response.json().then(function (data) {
	                var node = $(e.target).parent('.form-group').next();
	                if (data.data.length > 0) {
	                    (function () {
	                        var select = $(node).find('select');
	                        select.empty();
	                        node.removeClass('hidden');
	                        select.append($('<option value="">Select Region</option>'));
	                    ...
	                    node.find('select').attr('required', false);
...
/******/ });
//# sourceMappingURL=user_profile.bundle.js.map