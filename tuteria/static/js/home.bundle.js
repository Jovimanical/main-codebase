...
/***/ function(module, exports, __webpack_require__) {
	...
	window.initializeCallback = function HomePageGoogleCallback() {
	    ...
	    function responseCallback(autocomplete) {
	        ...
	        $('#location_search').val(place.name + ", " + place.vicinity);
	        $('#l_latitude').val(place.geometry.location.lat());
	        $('#l_longitude').val(place.geometry.location.lng());
	        $('#geocoded').val("true");
	        //Modal Form for mobile devices
	
	        var state = place.address_components[place.address_components.length - 2].long_name;
	        var searchTag = Utils.filterOptionByText($('#id_region'), state);
	        if (searchTag) {
	        ...
	        console.log($('#l_longitude').val());
	        var state = place.address_components[place.address_components.length - 2].long_name;
	        console.log(state);
	    }
	
	    Utils.GoogleAutoComplete($('#id_location'), responseCallback);
	    Utils.SkillAutoComplete($('#search_only_form input[name="query"]'), function () {});
	    var loc = $('#location_search');
	    if (loc.length) {
	        Utils.GoogleAutoComplete($('#location_search'), responseCallback);
	    }
	    //Modal Form for mobile devices
	};
	
	//Scrollable and fixed navbar on home page
	function homenav() {
	    if ($(document).width() > 992) {
	        $('body').css({ marginTop: '0' });
	    }
	
	    // $('#firstss').css({'margin-top':'0'});
	    $('.na').removeClass('navbar-fixed-top fade in');
	    $('.outer').addClass('transparent-nav');
	    $('.category_div').hide();
	...
	function realnav() {
	    $('.na').addClass('navbar-fixed-top fade in').css({ display: 'none' }).slideDown('fast');
	    // if ($(document).width() > 992) {
	    //     // $('body').css({marginTop: '100px'});
	    // }
	    $('.outer').removeClass('transparent-nav');
	    // $('#firstss').css({'margin-top':'48px'});
	    $('.category_div').show();
	    $('#hiring').show();
	    $('.top-search input').blur();
	}
	...
	$(function () {
	    ...
	    $('#search_only_form').on('shown.bs.modal', function () {
	        $('#search_only_form #id_location').focus();
	    });
	    $('#carousel').carousel({
	        interval: 5000
	    });
	    ...
	    var bh = Utils.getBloodhoundInstance('name', Urls['ajax_skills_all']());
	    bh.initialize();
	    Utils.ApplyTypeaheadToInput($('#skill_search'), 'name', 'name', bh, function (obj, datum, name) {
	        var skill = datum.name.split(" in ")[0].trim();
	        $(this).typeahead('val', skill);
	        console.log(obj);
	        console.log(datum);
