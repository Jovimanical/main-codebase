...
/***/ function(module, exports, __webpack_require__) {
	...
	var Spinner = __webpack_require__(127);
	...
	window.initializeCallback = function GCallback() {
	    ...
	    function responseCallback(autocomplete) {
	        ...
	        var optionTag = Utils.filterOptionByText($('.modal2 #id_region'), state);
	        optionTag.attr("selected", true);
	        var searchTag = Utils.filterOptionByText($('#search_only_form #id_region'), state);
	        if (searchTag) {
	            searchTag.attr("selected", true);
	        }
	        console.log(place.geometry.location);
	        $('#id_location').val(place.name + ", " + place.vicinity);
	        $('.modal2 #id_location').val(place.name + ", " + place.vicinity);
	        $('#search_only_form #id_location').val(place.name + ", " + place.vicinity);
	        $('#tablet_filter #id_location').val(place.name + ", " + place.vicinity);
	        $('#geocoded').val("true");
	        $('.modal2 #geocoded').val("true");
	        $('#search_only_form #geocoded').val("true");
	        $('#tablet_filter #geocoded').val("true");
	        $('#id_latitude').val(place.geometry.location.lat());
	        $('.modal2 #id_latitude').val(place.geometry.location.lat());
	        $('#search_only_form #id_latitude').val(place.geometry.location.lat());
	        $('#tablet_filter #id_latitude').val(place.geometry.location.lat());
	        $('#id_longitude').val(place.geometry.location.lng());
	        $('.modal2 #id_longitude').val(place.geometry.location.lng());
	        $('#search_only_form #id_longitude').val(place.geometry.location.lng());
	        $('#tablet_filter #id_longitude').val(place.geometry.location.lng());
	        $('#id_region').val("");
	        $('#tablet_filter #id_region').val("");
	        ...
	        // 		query:qs.query
	        // 	}
	        // 	$("#result-display").html('&nbsp;').load(Urls["ajax_search"]()+"?"+$.param(params));
	        // }
	        ajaxSearch();
	    }
	
	    Utils.GoogleAutoComplete($('#id_location'), responseCallback);
	    Utils.GoogleAutoComplete($('.modal2 #id_location'), responseCallback);
	    Utils.GoogleAutoComplete($('#tablet_filter #id_location'), responseCallback);
	    Utils.GoogleAutoComplete($('#search_only_form #id_location'), responseCallback);
	    Utils.GenericSkillSearchAutocomplete($('#mobile_search_form'), $('#mobile_search_form').closest('form'));
	    Utils.GenericSkillSearchAutocomplete($('#search_only_form input[name="query"]'), []);
	};
	window.openUrl = function (val) {
	    window.location.href = val;
	};
	
	window.openUrl2 = function (val) {
	    window.location.href = Urls['skill_search']() + '?category=' + val;
	};
	window.evaluateQuery = function (tags) {
	    window.location.href = prepareUrlForSearch(true, Urls['search']()) + "&tags=" + tags;
	    //loadSearchResult(null, null, new_url);
	};
	function ajaxSearch() {
	    ...
	    // var qq = window.location.pathname === Urls['search']() ? qs.query : $('#id_query').val();
	    if (isDesktop || isTablet) {
	        loadSearchResult();
	    }
	}
	function prepareUrlForSearch(skill_page_param, special_url) {
	    var sk;
	    var ck = $('#category_page').val();
	    ...
	    if (isDesktop) {
	        ...
	        params = {
	            ...
	            start_rate: $("#id_start_rate").val(),
	            end_rate: $("#id_end_rate").val(),
	            gender: $("#id_gender").val(),
	            is_teacher: $("#id_is_teacher").is(":checked") || "",
	            skill_page: sk,
	            days: $("#id_days_per_week").val(),
	            category_page: ck
	        };
	    }
	    if (isTablet) {
	        if (skill_page_param) {
	            ...
	            sk = $("#tablet_filter #skill_page").val();
	        }
	        params = {
	            location: $('#tablet_filter #id_location').val(),
	            longitude: $('#tablet_filter #id_longitude').val(),
	            latitude: $('#tablet_filter #id_latitude').val(),
	            query: $('#id_query').val(),
	            age: $('#tablet_filter #id_age').val(),
	            distance: $("#tablet_filter #id_distance").val(),
	            region: $("#tablet_filter #id_region").val(),
	            start_rate: $("#tablet_filter #id_start_rate").val(),
	            end_rate: $("#tablet_filter #id_end_rate").val(),
	            gender: $("#tablet_filter #id_gender").val(),
	            is_teacher: $("#tablet_filter #id_is_teacher").is(":checked") || "",
	            skill_page: sk,
	            days: $("#tablet_filter #id_days_per_week").val(),
	            category_page: ck
	        };
	    }
	    var url = special_url || Urls["ajax_search"]();
	    return url + "?" + $.param(params);
	}
	function loadSearchResult(is_pagination, extra_params, special_url) {
	    ...
	    var target = document.getElementById('base_search_layout');
	    // var target = document.body;
	    target.appendChild(spinner.el);
	    $('#demo-holder').removeClass('hidden');
	    var url = prepareUrlForSearch();
	
	    if (is_pagination) {
	        url += "&" + extra_params;
	        document.body.scrollTop = document.documentElement.scrollTop = 81;
	    }
	    var new_url = special_url || url;
	    $("#result-display").html('&nbsp;').load(new_url, function () {
	        $('#demo-holder').addClass('hidden');
	        spinner.stop();
	    });
	}
	/**handling pagination for desktop and tablets**/
	var isDesktop = window.matchMedia("(min-width: 993px)").matches;
	var isTablet = window.matchMedia("(min-width: 768px) and (max-width:992px)").matches;
	if (isDesktop || isTablet) {
	    $('#result-display').on('click', '.pagination', function (e) {
	        e.preventDefault();
	        var queryparams = $(e.target).attr("href").replace("?", "");
	        loadSearchResult(true, queryparams);
	    });
	}
	...
	$(function () {
	    ...
	    $('#result-display').on('click', '#toggle-price-type', function () {
	        ...
	        var state = true,
	            ...
	            m_text = $('.monthly_p'),
	            h_text = $('.hourly_p');
	        if (input_text === "Monthly Price") {
	            $(this).text("Hourly Price");
	            monthly.removeClass('hide');
	            m_text.removeClass('hidden');
	            hourly.addClass('hide');
	            h_text.addClass('hidden');
	        } else {
	            $(this).text("Monthly Price");
	            state = false;
	            hourly.removeClass('hide');
	            h_text.removeClass('hidden');
	            monthly.addClass('hide');
	            m_text.addClass('hidden');
	        }
	    });
	
	    function initializeSlider(slider, start_rate, end_rate, amount) {
	        var min_input = start_rate.val();
	        var max_output = end_rate.val();
	        ...
	        slider.slider({
	            ...
	            slide: function slide(event, ui) {
	                ...
	                start_rate.val(ui.values[0]);
	                end_rate.val(ui.values[1]);
	            }
	        });
	        var amount_text = '₦' + slider.slider("values", 0) + ' -  ₦' + slider.slider("values", 1);
	        if (slider.slider("values", 1) >= 20000) {
	            amount_text += "+";
	        }
	    ...
	    // initializeSlider($("#slider2"),$("#id_start_rate"),$("#id_end_rate"),$("#amount"));
	    // initializeSlider($(".modal2 #slider2"),$(".modal2 #id_start_rate"),$(".modal2 #id_end_rate"),$(".modal2 #amount"));
	    // initializeSlider($("#tablet_filter #slider2"),$("#tablet_filter #id_start_rate"), $("#tablet_filter #id_end_rate"),
	    // $("#tablet_filter #amount"));
	
	    $('#result-display').on('click', '#displayFilterBtn', function () {
	        $('#tablet_filter .filter-form-container').toggleClass("hidden");
	        // $(this).fadeOut(1000);
	    });
	    ...
	    function FireSpinner() {
	        ...
	        target = document.getElementById('result-display2');
	    ...
	    $('.extra-filters a').click(function () {
	        $('.additional_filters').toggleClass("hidden");
	        $(this).addClass("hidden");
	    });
	    /**Event handler when gender filter is selected **/
	    function GenderEvent(ff) {
	        ff.on('change', function () {
	            ajaxSearch();
	        });
	    }
	
	    GenderEvent($("#id_gender"));
	    GenderEvent($("#tablet_filter #id_gender"));
	    /**Event handler when age filter is selected**/
	    function AgeEvent(ff) {
	        ff.on('change', function () {
	            ajaxSearch();
	        });
	    }
	    ...
	    AgeEvent($("#id_age"));
	    AgeEvent($("#tablet_filter #id_age"));
	    /**Event handler when distance filter is selected **/
	    function DistanceEvent(ff, loc_val) {
	        ff.on('change', function () {
	            if (loc_val.val()) {
	                ajaxSearch();
	            } else {
	                var region = $('#id_region').val();
	                loc_val.val(region || 'Lagos');
	                ajaxSearch();
	            }
	        });
	    }
	
	    DistanceEvent($('#id_distance'), $("#id_location"));
	    DistanceEvent($('#tablet_filter #id_distance'), $("#tablet_filter #id_location"));
	    /**Event handler when end price filter is selected**/
	    function PriceMaxEvent(ff) {
	        ff.on('change', function () {
	            ajaxSearch();
	        });
	    }
	
	    PriceMaxEvent($("#id_end_rate"));
	    PriceMaxEvent($("#tablet_filter #id_end_rate"));
	
	    function PriceOptions(min_field, max_field) {
	        min_field.on('change', function () {
	            var selected_option = $(this).find("option:selected");
	            var max_choices = [];
	            switch (selected_option.val()) {
	                case "100":
	                    console.log(100);
	                    max_choices = ["500", "1000", "1500", "2000", "2500", "3000", "3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                case "500":
	                    console.log(500);
	                    max_choices = ["1000", "1500", "2000", "2500", "3000", "3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                case "1000":
	                    console.log(1000);
	                    max_choices = ["1500", "2000", "2500", "3000", "3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                case "1500":
	                    console.log(1500);
	                    max_choices = ["2000", "2500", "3000", "3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                case "2000":
	                    console.log(2000);
	                    max_choices = ["2500", "3000", "3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                case "2500":
	                    console.log(2500);
	                    max_choices = ["3000", "3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                case "3000":
	                    console.log(3000);
	                    max_choices = ["3500", "4000", "4500", "5000", "5000+"];
	                    break;
	                default:
	                    console.log("Default");
	                    break;
	            }
	            max_field.empty().append($("<option />").val("").text("Max price"));
	            $.each(max_choices, function (x, y) {
	                if (y == "5000+") {
	                    max_field.append($("<option />").val("20000").text('₦' + y));
	                } else {
	                    max_field.append($("<option />").val(y).text('₦' + y));
	                }
	            });
	    ...
	    PriceOptions($("#id_start_rate"), $("#id_end_rate"));
	    PriceOptions($("#tablet_filter #id_start_rate"), $("#tablet_filter #id_end_rate"));
	    PriceOptions($(".modal2 #id_start_rate"), $(".modal2 #id_end_rate"));
	    function IsTeacherEvent(field) {
	        field.on('change', function () {
	            ajaxSearch();
	        });
	    }
	
	    IsTeacherEvent($("#id_is_teacher"));
	    IsTeacherEvent($("#tablet_filter #id_is_teacher"));
	    IsTeacherEvent($('.modal2 #is_teacher'));
	    function RegionEvent(field) {
	        field.on('change', function () {
	            ajaxSearch();
	        });
	    }
	
	    RegionEvent($('#id_region'));
	    RegionEvent($('#tablet_filter #id_region'));
	    RegionEvent($('.modal2 #id_region'));
	
	    function DaysPerWeek(field) {
	        field.on('change', function () {
	            ajaxSearch();
	        });
	    }
	
	    DaysPerWeek($('#id_days_per_week'));
	    DaysPerWeek($('#tablet_filter #id_days_per_week'));
	    DaysPerWeek($('.modal2 #id_days_per_week'));
	    ...
	    Utils.GetLongitudeAndLatitudeAndSubmit($(".modal2 #filterForm"), $(".modal2 #id_location"), $(".modal2 #id_longitude"), $(".modal2 #id_latitude"), $('.modal2 #id_region'), function () {
	        return geocoder;
	    });
	    Utils.GetLongitudeAndLatitudeAndSubmit($("#search_only_form #filterForm"), $('#search_only_form #id_location'), $("#search_only_form #id_longitude"), $('#search_only_form #id_latitude'), $('#search_only_form #id_region'), function () {
	        return geocoder;
	    });
	
	    $('#operaFilterBtn').on('click', function () {
	        // if(isOperaMini){
	        $("#operamini_filter").toggleClass("hidden");
	    });
	    $('#operamini_filter .cancel_button').on('click', function () {
	        $("#operamini_filter").toggleClass("hidden");
	    });
	});
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/******/ });
//# sourceMappingURL=category_featured.bundle.js.map