...
/***/ function(module, exports, __webpack_require__) {
	...
	var addr_type = {
		HOME_ADDRESS: 1, TUTOR_ADDRESS: 2
	};
	
	var LocationsModel = (function () {
		function LocationsModel(loc) {
			_classCallCheck(this, LocationsModel);
	
			this.tutor_address = loc.filter(function (x) {
				return x.addr_type == addr_type.TUTOR_ADDRESS;
			});
			var tutor_address = this.tutor_address[0];
			this.vicinity = tutor_address ? tutor_address.vicinity : null;
			this.latitude = tutor_address ? parseFloat(tutor_address.latitude) : null;
			this.longitude = tutor_address ? parseFloat(tutor_address.longitude) : null;
			this.state = tutor_address ? tutor_address.state : null;
		}
	
		_createClass(LocationsModel, [{
			key: 'distance',
			value: function distance(coordinate) {
				var deg2rad = function deg2rad(deg) {
					return deg * (Math.PI / 180);
				};
				var R = 6371; // Radius of the earth in km
				var dLat = deg2rad(coordinate.latitude - this.latitude); // deg2rad below
				var dLon = deg2rad(coordinate.longitude - this.longitude);
	...
	var TutorSkill = (function () {
		function TutorSkill(data) {
			_classCallCheck(this, TutorSkill);
	
			this.location = new LocationsModel(data.tutor.location_set);
			this.first_name = data.tutor.first_name;
			this.price = parseFloat(data.price);
			this.heading = data.heading;
			this.monthly_price = parseFloat(data.monthly_price);
			this.education = data.tutor.education_set.map(function (x) {
				return x.degree;
			}).join(",");
			this.gender = data.tutor.profile.gender;
			this.dob = new Date(data.tutor.profile.dob);
			this.image_url = data.image_url;
			this.url = data.url;
			this.reveiws_count = data.reveiws_count;
		}
	
		_createClass(TutorSkill, [{
			key: 'savers',
			value: function savers() {
				return 23;
			}
		}], [{
			key: 'play',
			...
			key: 'getTutorSkills',
			value: function getTutorSkills(query) {
				var that = this;
				var dfd = $.Deferred();
				$.get(Urls['ajax_search']() + query, function (data) {
					console.log(data);
					var resolve = data.map(function (x) {
						return new TutorSkill(x);
					});
					dfd.resolve(resolve);
				});
				return dfd.promise();
	...
	angular.module('TuteriaSearch', []).controller('SearchCtrl', ['$scope', function ($scope) {
		var demo = {
			"tutor": {
				"education_set": [{
					"degree": "M.Sc."
				...
				"profile": {
					"gender": "M",
					"dob": "1989-07-09",
					"public_id": "profile_pics/ritadqmpdlrl6uji10tb"
				},
				"location_set": [{
					"vicinity": "Surulere",
					"state": "Lagos",
			...
			"price": "1500.00",
			"image_url": "http://res.cloudinary.com/iolab/image/upload/v1433946675/profile_pics/ritadqmpdlrl6uji10tb.png",
			"heading": "lorem ipsum is the way forward in life to do good 0",
			"monthly_price": 72000.0,
			"url": "/biolao/lorem-ipsum-is-the-way-forward-in-life-to-do-good-0/",
			"reveiws_count": 0
		};
		console.log(new TutorSkill(demo));
		$scope.coordinate = true;
		$scope.result = [new TutorSkill(demo)];
	}]);
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/******/ });
//# sourceMappingURL=category_smart.bundle.js.map