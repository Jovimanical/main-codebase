			...
			/* WEBPACK VAR INJECTION */ (function($) {
				...
				function LoginInitialize() {
					...
					var beforeSubmit = function beforeSubmit(data) {
						...
						$("#login_form")
							...
							.val(Utils.getCookie("csrftoken"));
					};
					var success = function success(data) {
						console.log(data);
						window.location.reload();
					};
					Utils.FormInitialize(
						$("#login_form"),
						beforeSubmit,
						success,
						null,
				...
				function ResetInitialize(regular) {
					...
					var success = function success(data, xhr) {
						form
							.find(".invalid-form-error-message")
							.html("Email Sent!")
							.addClass("filled");
						console.log(xhr);
					};
				...
				function SignUp(no_phone, signupCallback) {
					...
					var errorCallback = function errorCallback(xhr, err) {
						...
						for (var key in error) {
							errorNode.append(
								'<p class="text-danger">' + error[key][0] + "</p>"
							);
						}
					};
					if (!isOperaMini) {
						//debugger;
						Utils.formSubmission(
							signup_form,
							beforSubmit,
							successCallback,
							errorCallback,
					...
					$("#authModal").on("show.bs.modal", function(event) {
						...
						function displaySignupForm() {
							...
							modal_subtitle.html(
								'<a href="' +
									Urls["account_signup"]() +
									'">Join</a> Tuteria to connect with quality tutors and clients'
							);
						}
				...
				function AutocompleteSearch() {
					...
					var BH = Utils.getBloodhoundInstance(
						"name",
						Urls["ajax_skills_all"]()
					);
					BH.initialize();
					Utils.ApplyTypeaheadToInput(
						$("#nav-skill-input"),
						"name",
						"name",
						BH,
					...
					$(".share-popup").click(function(e) {
						...
						switch (domain) {
							case "www.facebook.com":
								$.get(
									Urls["button_clicks"]() + "?button_type=facebook",
									function() {}
								);
							...
							case "twitter.com":
								$.get(
									Urls["button_clicks"]() + "?button_type=twitter",
									function() {}
								);
							...
							case "plus.google.com":
								$.get(
									Urls["button_clicks"]() + "?button_type=google",
									function() {}
								);
		...
		/***/ 3: /***/ function(module, exports, __webpack_require__) {
			/* WEBPACK VAR INJECTION */ (function($) {
				...
				function ParsleyConfig() {
					...
					window.ParsleyConfig = {
						...
						i18n: {
							en: {
								valid_phone:
									"Phone number is invalid. should be of the form +234 703 520 9576",
								same_phone_number: "Phone number is not the same",
								schedule_time:
									"End time must be greater than start time with at least an hour",
								...
								validate_word_count:
									"Please input at least 300 words. No unnecessary spaces please."
							}
						}
					};
				}
				...
				var parsleyConfig = {
					...
					excluded:
						"input[type=text]:hidden, input[type=select]:hidden, textarea:hidden"
				};
				...
				function ApplyTypeaheadToInput(input, name, key, bhInstance, callback) {
					...
					input
						.typeahead(generic_options, TypeAheadConfig(name, key, bhInstance))
						.on("typeahead:selected", callback);
				}
				...
				function SkillAutoComplete(input, cb) {
					...
					ApplyTypeaheadToInput(input, "name", "name", BH, function(
						obj,
						datum,
						name
					) {
				...
				function GeocodeFunction(
					...
					geocoder.geocode({ address: full_address }, function(
						results,
						status
					) {
				...
				function GetLongitudeAndLatitudeAndSubmit(
					filterform,
					location_field,
					longitude_field,
					latitude_field,
	...
	*/
			...
			(function(factory) {
				...
				Plugin.prototype = {
					...
					_setInstanceCountryData: function _setInstanceCountryData() {
						...
						if (this.options.onlyCountries.length) {
							...
							for (i = 0; i < allCountries.length; i++) {
								if (
									$.inArray(allCountries[i].iso2, this.options.onlyCountries) !=
									-1
								) {
						...
						if (this.isGoodBrowser) {
							...
							if (newNumericChar) {
								...
								val =
									...
									newNumericChar +
									val.substring(selectionEnd, originalLen);
						...
						this.telInput.on("blur" + this.ns, function() {
							if (that.options.autoHideDialCode) {
								...
								if (startsPlus) {
									...
									if (
										!numeric ||
										that.selectedCountryData.dialCode == numeric
									) {
										that.telInput.val("");
									}
								}
						...
						for (var i = 0; i < this.countries.length; i++) {
							if (this._startsWith(this.countries[i].name, query)) {
								var listItem = this.countryList
									.children(
										"[data-country-code=" + this.countries[i].iso2 + "]"
									)
									.not(".preferred");
					...
					_getCountryData: function _getCountryData(
						countryCode,
					...
					_selectFlag: function _selectFlag(countryCode) {
						...
						this.selectedCountryData = this._getCountryData(
							countryCode,
							false,
							false
						);
					...
					_updatePlaceholder: function _updatePlaceholder() {
						if (window.intlTelInputUtils && !this.hadInitialPlaceholder) {
							var iso2 = this.selectedCountryData.iso2,
								...
								placeholder = intlTelInputUtils.getExampleNumber(
									iso2,
									this.options.nationalMode,
									numberType
								);
					...
					getCleanNumber: function getCleanNumber() {
						if (window.intlTelInputUtils) {
							return intlTelInputUtils.formatNumberE164(
								this.telInput.val(),
								this.selectedCountryData.iso2
							);
						}
	...
	*/
			...
			var MobileEsp = {
				...
				DetectKindle: function DetectKindle() {
					if (
						this.uagent.search(this.deviceKindle) > -1 &&
						!this.DetectAndroid()
					)
						return true;
					else return false;
		...
		/***/ 11: /***/ function(module, exports, __webpack_require__) {
			...
			/* WEBPACK VAR INJECTION */ (function($) {
				...
				window.ParsleyExtend = $.extend(window.ParsleyExtend, {
						...
						return this._asyncValidateForm(undefined, event).done(function() {
							...
							if (!that.submitEvent.isDefaultPrevented())
								that.$element.trigger(
									$.extend($.Event("submit"), { parsley: true })
								);
						});
					},
					...
					_handleRemoteResult: function _handleRemoteResult(
						validator,
						xhr,
						deferred
					) {
				...
				!(function(factory) {
					...
					ParsleyAbstract.prototype = {
						...
						validateThroughValidator: function validateThroughValidator(
							value,
							constraints,
							priority
						) {
					...
					var Validator = (function() {
						...
						Validator.prototype = {
							...
							_validateObject: function _validateObject(
								object,
								constraint,
								group
							) {
							...
							_validateBindedObject: function _validateBindedObject(
								object,
								group
							) {
						...
						Constraint.prototype = {
							...
							check: function check(object, group) {
								...
								for (var property in this.nodes) {
									...
									var constraints = _isArray(constraint)
										? constraint
										: [constraint];
					...
					Validator =
						"undefined" !== typeof Validator
							...
							? module.exports
							: null;
					var ParsleyValidator = function ParsleyValidator(
						validators,
						catalog
					) {
					...
					ParsleyValidator.prototype = {
						...
						updateValidator: function updateValidator(
							name,
							fn,
							priority,
							requirementsTransformer
						...
						validators: {
							...
							type: function type(_type) {
								...
								switch (_type) {
									...
									default:
										throw new Error(
											"validator type `" + _type + "` is not supported"
										);
								}
					...
					ParsleyUI.prototype = {
						...
						manageErrorsMessages: function manageErrorsMessages(
							fieldInstance,
							diff
						) {
							if (
						...
						_insertErrorWrapper: function _insertErrorWrapper(fieldInstance) {
							...
							if ("string" === typeof fieldInstance.options.errorsContainer) {
								...
								"function" === typeof fieldInstance.options.errorsContainer
							)
								$errorsContainer = fieldInstance.options.errorsContainer(
									fieldInstance
								);
							if (
						...
						manageFailingFieldTrigger: function manageFailingFieldTrigger(
							fieldInstance
						) {
					...
					var ParsleyOptionsFactory = function ParsleyOptionsFactory(
						defaultOptions,
						globalOptions,
						userOptions,
						namespace
					...
					var ParsleyForm = function ParsleyForm(element, OptionsFactory) {
						...
						this.__id__ = ParsleyUtils.hash(4);
						if (
							"OptionsFactory" !== ParsleyUtils.get(OptionsFactory, "__class__")
						)
					...
					ParsleyForm.prototype = {
						...
						validate: function validate(group, force, event) {
							...
							$.emit(
								"parsley:form:" + (this.validationResult ? "success" : "error"),
								this
							);
					...
					var ConstraintFactory = function ConstraintFactory(
						...
						var getPriority = function getPriority() {
							if (
								"undefined" !== typeof parsleyField.options[name + "Priority"]
							)
				...
				ParsleyField.prototype = {
					...
					_bindConstraints: function _bindConstraints() {
						...
						for (var i = 0; i < this.constraints.length; i++)
							if (false === this.constraints[i].isDomConstraint) {
								constraints.push(this.constraints[i]);
								constraintsByName[this.constraints[i].name] = this.constraints[
									i
								];
							}
						this.constraints = constraints;
		...
		/***/ 143: /***/ function(module, exports, __webpack_require__) {
			/* WEBPACK VAR INJECTION */ (function($) {
				...
				$(function() {
					$("#pricing-form").bind("onShowForm", function() {
						Utils.parsleyfyForm($("#pricing-form"));
					});
					Utils.parsleyfyForm($("#pricing-form"));
				});
				/* WEBPACK VAR INJECTION */
		...
		/***/ 145: /***/ function(module, exports) {
			...
			(function($) {
				...
				function TagsInput(element, options) {
					...
					this.$input.get(0).style.cssText =
				...
				TagsInput.prototype = {
					...
					build: function build(options) {
						...
						if (self.options.typeaheadjs) {
							...
							self.$input.typeahead(null, typeaheadjs).on(
								"typeahead:selected",
...
//# sourceMappingURL=pricing_form.bundle.js.map
