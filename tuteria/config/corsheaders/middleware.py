...
class CorsMiddleware(MiddlewareMixin):
    ...
    def _https_referer_replace(self, request):
        ...
        if (
            ...
            url = urlparse(origin)
            if (
                not settings.CORS_ORIGIN_ALLOW_ALL
                and self.origin_not_found_in_white_lists(origin, url)
            ):
                return
        ...
        if self.is_enabled(request) and origin:
            ...
            response[ACCESS_CONTROL_ALLOW_ORIGIN] = (
                ...
                if (
                    ...
                    and not settings.CORS_ALLOW_CREDENTIALS
                )
                else origin
            )
    ...
    def origin_not_found_in_white_lists(self, origin, url):
        return (
            url.netloc not in settings.CORS_ORIGIN_WHITELIST
            and not self.regex_domain_match(origin)
        )
